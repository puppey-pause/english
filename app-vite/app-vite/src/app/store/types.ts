...
import type { Song, SongWork } from "@/entities/song";
...
export interface SongSlice {
  songWork: Record<string, SongWork>;
  ownSongs: Song[];
  splitLyrics: (songId: string, raw: string) => void;
  setSongLine: (songId: string, field: "tr" | "nb", index: number, value: string) => void;
  resetLyrics: (songId: string) => void;
  addOwnSong: (artist: string, title: string) => string;
}

export type AppState = UiSlice & ProgressSlice & DeckSlice & MistakeSlice & SongSlice;
